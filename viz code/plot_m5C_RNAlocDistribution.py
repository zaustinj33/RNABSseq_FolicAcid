...
total_file = "allm5C_libraries_filteredAnno.csv"
...
for set in conditions:
    print(set)
    for file in glob.glob("**/" + set + "_allOverlap.csv", recursive=True):
...
site_bin_interp['name'] = pd.Series(site_bin_interp['level_0'].str.match(r'^p.*'))
site_bin_interp['fraction'] = np.where(site_bin_interp['name'] == True, "Polysome", "Total")
#%%
from scipy.interpolate import interp1d
site_bin_interp_test = site_bin_interp[site_bin_interp['fraction'] == 'Total']
# Interpolate points
...
plt.xticks([])
plt.savefig("site_distribution_condition_Total.png",bbox_inches='tight', dpi=200, transparent=True)
plt.show()
...
# %%
#test =
sns.lineplot(data=test, x='level_1', y=0, hue='nameL', ci=None, palette='bright')
plt.axvline(5, 1, 0, linestyle='--')
plt.axvline(17, 1, 0, linestyle='--')
plt.xlim(0, 25)
# plt.legend([], frameon=False)
# plt.savefig("site_distribution_condition_trueOverlap.png",bbox_inches='tight', dpi=400, transparent=True)
plt.show()
...
sns.violinplot(data=plot_dist_ML, linewidth=3, palette='husl', cut=True)
plt.ylim(-0.1, 1.1)

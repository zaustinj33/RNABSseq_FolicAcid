...
prep_annotations <- function (preDMS_file){
  ...
  name <- "all_merged"
  overlap <- preDMS_file
  overlap$chrom <- gsub(" .*$","", preDMS_file$group)
  overlap$refPos <- gsub("^.* ","", preDMS_file$group)
  overlap_genome <- data.frame(chrom=overlap$chrom, position=overlap$refPos, group=overlap$group)
  #annotation

  dat1 <- overlap_genome
  dat2 <- read.delim(paste0(getwd(),"/../RNABS_projects/Chapter_5/mm10_anno_79_genes.txt"))
...
write.csv(all_merged_anno,'allm5C_libraries_filteredDepthAnno.csv', row.names = F)
